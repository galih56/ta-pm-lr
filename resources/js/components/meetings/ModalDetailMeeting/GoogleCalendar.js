...
const GoogleCalendar  = ({isEdit,meeting,setMeeting,detailProject,saveChanges}) => {
                            ...
                            gapi.load('client:auth2', function(){
                                gapi.auth2.authorize({
                                    ...
                                    getCalendar(function(calendars){
                                        var calendarWithProjectName = checkIfProjectCalendarExist(calendars,detailProject.title);
                                        if(calendarWithProjectName){
                                            var calendar_info=JSON.parse(meeting.member?.google_calendar_info)
                                            if(calendar_info) deleteEvent(calendarWithProjectName.id,calendar_info.id);
                                        }
                                        setLoading(false);
                                    });
                                });
